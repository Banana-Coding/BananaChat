    ...
    public static void parse(String message, Client client, Channel channel) {
        ...
        if (cmd.equals("a")) {
            ...
            if (channelTo == null) {
                ...
                client.sendButlerMessage(channel.getName(), String.format("Du bist doch schon im Channel %s.", ch));
            } else if (!client.isModerator() && channelTo.countClients() == channelTo.getSize()) {
            ...
            client.send(PacketCreator.openURL("http://forum.knuddels.de/categories.html", "forum"));
            ...
            if (nickname.equalsIgnoreCase(client.getName())) {
                client.sendButlerMessage(channel.getName(), "Sie können sich nicht selbst hinauswerfen.");
                return;
            }
            ...
            if (target == null) {
                client.sendButlerMessage(channel.getName(), String.format("%s ist mir unbekannt.", nickname));
            } else if (target.getRank() >= client.getRank()) {
                client.sendButlerMessage(channel.getName(), String.format("Sie haben nicht genügend Rechte um %s hinauszuwerfen.", target.getName()));
            } else {
            ...
            if (target == null) {
                ...
                channel.broadcastAction(">", String.format("°>_h%s|/serverpp \"|/w \"<° kann einfach nicht anders und gibt °>_h%s|/serverpp \"|/w \"<° einen langen Kuss.", client.getName(), target.getName()));
            }
            ...
            if (target == null) {
                ...
                client.sendButlerMessage(channel.getName(), "Knuddeln kann man nur, wenn man auch Knuddels hat, und die fehlen dir leider.");
            } else {
                client.setKnuddels(client.getKnuddels() - 1);
                target.setKnuddels(target.getKnuddels() + 1);
                channel.broadcastAction(">", String.format("°>_h%s|/serverpp \"|/w \"<° hat °>_h%s|/serverpp \"|/w \"<° scheinbar richtig gern und °RR°knuddelt°BB° eifrig drauf los.", client.getName(), target.getName()));
            }
            ...
            Popup popup = new Popup("Umfrage", "Umfrage erstellen", "Gib unten eine Frage und mindestens zwei verschiedene Antwortmöglichkeiten - durch Semikolons getrennt - ein. Jeder Chatter hat dann die Möglichkeit die Umfrage innerhalb von einer Minute zu beantworten und dabei 5 Knuddels abzusahnen, welche unter allen Teilnehmern verlost werden.", 400, 300);
            ...
            whois.append(String.format(" hat sich am _%s_", target.getRegistrationDate()));
            whois.append(String.format(" um %s", target.getRegistrationTime()));
            whois.append(String.format(" bei Knuddels registriert und seitdem schon _%s_ Minuten hier verbracht.##", nf.format(target.getOnlineTime() / 60)));

            if (online) {
                Channel ch = target.getChannel();

                if (ch == null) {
                    whois.append(String.format("°>py_g.gif<°°%%05°%s ist im Moment im Channel _? °E°ONLINE°r°_!°%%00°##", nickname));
                } else {
                    whois.append(String.format("°>py_g.gif<°°%%05°%s ist im Moment im Channel _°>_h%s|/go \"|/go +\"<° °E°ONLINE°r°_!°%%00°##", nickname, ch.getName()));
                }
            } else {
                whois.append(String.format("°>py_r.gif<°°%%05°%s ist im Moment _OFFLINE_!°%%00°##", nickname));
            }

            whois.append(String.format("%s hat...°%%35°", nickname));
            whois.append(String.format("°>gt.gif<° _°R°%s°r°_ %s...#", nf.format(target.getKisses()), target.getKisses() == 1 ? "Knutschfleck" : "Knutschflecken"));
            whois.append(String.format("°>gt.gif<° und kann noch _°R°%sx°r° knuddeln_!", nf.format(target.getKnuddels())));
            whois.append("°%00°##°-°");
            ...
            if (client.isModerator()) {
                whois.append("°-°");
                whois.append(String.format("_IP-Adresse_:°%%35°%s°%%00°#", target.getIPAddress()));
            }
