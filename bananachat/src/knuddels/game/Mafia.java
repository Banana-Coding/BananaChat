...
public class Mafia implements Game, Runnable {
	...
	private void action() {
		...
		if (status == 1) {
			synchronized (players) {
				if (players.size() < 5) {
					channel.broadcastButlerMessage(String
							.format("Noch können wir leider nicht anfangen. Wir haben _erst %s Spieler_, brauchen aber _mindestens 5 Spieler_. Wer kein Angsthase ist, meldet sich bei mir °R°privat°r°.",
									players.size()));
					status++;
				} else {
					status = 3;
			...
			synchronized (players) {
				if (players.size() < 5) {
					channel.broadcastButlerMessage(String
							.format("Leider findet unsere Mafiarunde nicht statt. Mindestens _5 Spieler benötigen_ wir, es haben sich aber _nur %s Spieler_ gemeldet.",
									players.size()));
					status = 0;
				} else {
					status++;
	...
	public boolean parsePrivateMessage(List<Client> targets, String message,
		...
		if (status > 2 && !client.isModerator()) {
			client.sendButlerMessage(channel.getName(),
					"Diese Funktion steht dir hier nicht zur Verfügung.");
			return false;
		}
			...
			if (status == 1 || status == 2) {
				...
				synchronized (players) {
					if (players.containsKey(client.getName())) {
						...
						players.put(client.getName(),
								new MafiaPlayer(client.getName()));
						client.sendButlerMessage(channel.getName(),
								"Sie sind nun für die neue Mafiarunde angemeldet.");
	...
	public void onLeave(Client client) {
		synchronized (players) {
			if (players.containsKey(client.getName())) {
				MafiaCharacter character = players.get(client.getName())
						.getCharacter();
